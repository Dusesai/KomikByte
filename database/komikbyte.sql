-- KomikByte database schema
-- Run this file in phpMyAdmin or the MySQL command line before opening the app.
CREATE DATABASE IF NOT EXISTS komikbyte CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE komikbyte;
...
-- Demonstration catalog. Chapter content is intentionally plain text for the academic prototype.
...
INSERT INTO chapters (comic_id, chapter_number, title, content, price) VALUES
(1, 1.0, 'Signal in the Rain', 'The city never sleeps. Neon reflected off every flooded street as Kai received a delivery with no return address.\n\nAt its center was an old steel sheath, warm despite the midnight rain.', 0.00),
(1, 2.0, 'Memory Blade', 'The sheath opened with a sound like distant thunder. A voice spoke from the blade: “I remember the last person who carried me.”\n\nKai ran before the patrol drones saw the light.', 0.00),
(1, 3.0, 'Circuit Shrine', 'Beneath the elevated rails, Kai found the shrine drawn on the delivery map. Every candle was electric. Every prayer was encrypted.', 5.00),
(1, 4.0, 'The Borrowed Name', 'A stranger called Kai by a name he had never heard — and the blade answered for him.', 10.00),
(2, 1.0, 'The Island Above', 'Lina’s first map was a mistake. The mountain she drew rose into the sky, and by morning it was waiting above her village.', 0.00),
(2, 2.0, 'Cloud Ferry', 'The ferry captain only asked for one thing before the voyage: a promise never to look down.', 5.00),
(3, 1.0, 'A Small Light', 'When the lamps went out, Tala lit the last lantern. Its gentle glow reached farther than anyone expected.', 0.00),
(3, 2.0, 'Morning Path', 'The townspeople followed the lantern through the forest, discovering that dawn had been close all along.', 5.00);
